...
export function drawCoil(canvas: any){
...
export function drawFrameThumb(canvas: any, img: any){
  const dpr = window.devicePixelRatio || 1;
...
let expandHideTimer: any = null;
export function showExpand(roll: Roll, sec: any){
...
export function renderTray(){
  const wrap = $('#rolls');
  wrap.querySelectorAll('.roll').forEach((n: any)=>n.remove());
