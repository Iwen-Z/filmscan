...
// 入库记录形态(纯可序列化)
interface StoredRoll { id: number; name: string; filmType: FilmType; shots: Blob[]; }
...
export async function persistRoll(roll: Roll): Promise<void> {
  ...
  try {
    ...
    const rec: StoredRoll = { id: roll.id, name: roll.name, filmType: roll.filmType, shots };
...
export async function loadAllRolls(): Promise<Array<{ id: number; name: string; filmType: FilmType; shots: Blob[] }>> {
  try {
    const db = await openDB();
    try {
